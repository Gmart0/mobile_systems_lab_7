	...
	func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
		if searchText.count >= 3 {
			DispatchQueue.global().sync {
				...
				if let path = "http://www.omdbapi.com/?apikey=7e9fe69e&s=\(searchText)&page=1".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed), let URL = URL(string: path)
